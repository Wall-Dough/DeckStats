...
internal static class ModConfigBridge
{
    ...
    private static string MOD_ID = "DeckStats";
    ...
    private static void Register()
    {
        ...
        try
        {
            ...
            if (registerMethod.GetParameters().Length == 4)
            {
                ...
                {
                    MOD_ID,          // Must match your mod's ID
        ...
        catch (Exception e)
        {
            ...
            GD.PrintErr($"DeckStats ModConfig registration failed: {e}");
        }
    }
    ...
    internal static T GetValue<T>(string key, T fallback)
    {
        ...
        try
        {
            var result = _apiType!.GetMethod("GetValue", BindingFlags.Public | BindingFlags.Static)
                ...
                ?.Invoke(null, new object[] { MOD_ID, key });
    ...
    internal static void SetValue(string key, object value)
    {
        ...
        try
        {
            _apiType!.GetMethod("SetValue", BindingFlags.Public | BindingFlags.Static)
                ?.Invoke(null, new object[] { MOD_ID, key, value });
        }
        catch { }
    }
    ...
    private static Array BuildEntries()
    {
        ...
        //     }));
        // }));
        foreach (string statName in DeckStats.ALL_STATS)
        {
            if (statName == DeckStats.NONE) continue;
            string statNameFormatted = statName.Replace('_', ' ');
            // string description = "Show " + statName;
            list.Add(Entry(cfg =>
            {
                Set(cfg, "Key", statName);
                Set(cfg, "Label", statNameFormatted);
                Set(cfg, "Labels", L(statNameFormatted));
                Set(cfg, "Type", EnumVal("Toggle"));
                Set(cfg, "DefaultValue", true);
                // Set(cfg, "Description", description);
                // Set(cfg, "Descriptions", L(description));
                Set(cfg, "OnChanged", new Action<object>(v =>
                {
                    DeckStats.ShowStat(statName, Convert.ToBoolean(v));
                    DeckStats.UpdateColumns();
                }));
            }));
        }
    ...
    private static Dictionary<string, string> L(string en)
        => new() { ["en"] = en };
